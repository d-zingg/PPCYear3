...
export default function PublicProfile() {
  ...
  useEffect(() => {
    ...
    const storedUserId = localStorage.getItem('ppc_userId') || localStorage.getItem('ppc_email')
    setViewerUserId(storedUserId)
  }, [])

  // Find the user
  const profileUser = allUsers.find((u) => u.id === userId || u.email === userId)
  const userPosts = profileUser && getPostsByUser ? getPostsByUser(profileUser.email || profileUser.id) : []
    ...
    <div className="min-h-screen p-8 bg-gray-50">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white p-6 rounded-lg shadow mb-8">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 capitalize">{profileUser.role || 'User'}</p>
            </div>
          </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          <h2 className="text-2xl font-bold mb-6">{profileUser.name || profileUser.email}'s Posts</h2>
              ...
              {userPosts.map((post) => (
                ...
                >
                  ...
                  <div className="flex gap-6 mt-4">
                    <button
                      onClick={() => viewerUserId && toggleLike(post.id, viewerUserId)}
                    ...
                    <button
                      onClick={() => viewerUserId && toggleFavorite(post.id, viewerUserId)}
                      className="hover:text-yellow-600 cursor-pointer flex items-center gap-1"
                    >
                      {(post.favoritedBy || []).includes(viewerUserId) ? '★' : '☆'} <span>{post.favorites || 0}</span>
                    </button>
                  </div>
