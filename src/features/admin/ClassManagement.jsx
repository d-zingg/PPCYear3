...
const mockTeachers = [
  {
    id: "u3",
    name: "Charlie Davis (Teacher)",
    email: "charlie@school.edu",
    role: "teacher",
  },
  {
    id: "u9",
    name: "Laura Miller (Teacher)",
...
const mockStudentsPerClass = {
  c1: [
    { id: "stu1", name: "Alice Johnson" },
    { id: "stu2", name: "Bob Smith" },
  ],
  c2: [
    { id: "stu3", name: "Eve Adams" },
    { id: "stu4", name: "Grace Hall" },
  ],
...
const useMockClasses = () => ({
  ...
  updateClass: (id, data) =>
    console.log(`[MOCK] Updating class ${id} with:`, data),
...
export default function ClassManagement() {
  ...
  const teachers = allUsers
    ...
    : mockTeachers.filter((u) => u.role === "teacher");
  ...
  const handleSubmit = (e) => {
    ...
    if (editingClass) {
      ...
      setLocalClasses((prev) =>
        prev.map((c) => (c.id === editingClass ? { ...c, ...formData } : c)),
      );
  ...
  const handleDelete = (classId, name) => {
    if (
      window.confirm(
        `Are you sure you want to delete class: ${name}? This cannot be undone.`,
      )
    ) {
      deleteClass(classId);
        ...
        >
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <h3 className="text-xl font-bold mt-6 pt-4 border-t">
                📝 Enrolled Students ({selectedClass.totalStudents})
              </h3>
              <div className="max-h-60 overflow-y-auto border rounded p-3">
                <ul className="space-y-2">
                  {(getStudentsInClass(selectedClass.id) || []).length > 0 ? (
                    (getStudentsInClass(selectedClass.id) || []).map(
                      (student) => (
                        <li
                          key={student.id}
              ...
              <button className={btnSecondary + " w-full mt-4"}>
                Manage Enrollment (Mock Action)
              </button>
            </div>
          </div>
        </div>
